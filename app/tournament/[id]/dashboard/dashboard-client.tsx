...
export default function TournamentDashboardClient({ tournamentId, initialData }: DashboardClientProps) {
  const [tournament, setTournament] = useState<Tournament>(initialData.tournament);
  ...
  const processData = useCallback((data: DashboardInitialData) => {
    // Process Matches
    const currentMatches = data.matches || [];
    
    // Get current round
    const maxRound = currentMatches.length > 0 
      ? Math.max(...currentMatches.map((m) => m.round_number || 1))
      : 1;
    
    setCurrentRound(maxRound);

    // Get current round matches for timer
    const currentRoundMatches = currentMatches.filter((m) => m.round_number === maxRound);
    ...
    if (timerMatch) {
      ...
      updateTimerDisplay(timerMatch, data.tournament.round_duration_minutes);
    }
    ...
    data.participants.forEach((p) => {
      ...
      const rawResult = p.result ? p.result.toLowerCase() : null;
      const normalizedResult = (rawResult === 'win' || rawResult === 'loss' || rawResult === 'draw') ? rawResult as 'win' | 'loss' | 'draw' | null : null;
      ...
      participantsByMatch.get(p.match_id)!.push(normalizedParticipant);
    });
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch all match participants using inner join
      const { data: allParticipants } = await supabase
        .from('match_participants')
        .select(`
          id, match_id, profile_id, result, games_won,
      ...
      // Profiles are mostly static, but we could re-fetch if needed.
      // For now, reusing initialData profiles is fine for name display.
      // If new players can be added mid-tournament, we'd need to fetch them.
      
      const refreshedData: DashboardInitialData = {
  ...
  }, [tournamentId, tournament, totalPlayers, initialData.profiles, processData]);
  ...
  // Timer Handlers (Client wrappers around Server Actions)
