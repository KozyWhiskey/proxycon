...
export default async function EventsPage() {
  ...
  const activeEvents = myEvents.filter((e: any) => e.is_active !== false);
  const pastEvents = myEvents.filter((e: any) => e.is_active === false);

  const EventCard = ({ event, isPast = false }: { event: any, isPast?: boolean }) => (
    <Link key={event.id} href={`/events/${event.id}`} className="group block h-full">
      <Card className={`glass-panel h-full transition-all duration-300 overflow-hidden relative flex flex-col ${isPast ? 'opacity-60 hover:opacity-100 border-white/5 hover:border-white/20' : 'hover:border-primary/50'}`}>
        ...
        <CardContent className="flex-1 flex flex-col justify-between">
          ...
          <div className="mt-6 flex items-center text-primary text-xs font-bold uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0 duration-300">
              {isPast ? 'View Past Event' : 'Enter Dashboard'} <ArrowRight className="w-3 h-3 ml-2" />
          </div>
        </CardContent>
      </Card>
    </Link>
  ...
  return (
    <main className="min-h-screen bg-background pb-24">
      ...
      <div className="max-w-7xl mx-auto p-4 space-y-12">
        ...
        {pastEvents.length > 0 && (
          <section>
            <h2 className="text-lg font-bold text-muted-foreground/60 uppercase tracking-widest font-heading mb-6 px-1 flex items-center gap-2">
